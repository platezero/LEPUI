  ...
  <v-layout row wrap>
            ...
            >
              <template slot="items" slot-scope="props">
                ...
                <td class="text-xs-center">
                  <v-icon @click="editItem(props.item)">search</v-icon>
                </td>
    ...
    <v-dialog v-model="dialog_item" max-width="500px">
      <v-card>
        ...
        <v-card-text>
          <v-container grid-list-md>
            <v-layout wrap>
              ...
              <v-flex xs12>
                <v-layout align-center justify-center row fill-height>
                  <v-icon @click="showChequeDetail(editedItem.rpmLandAmtDoc)">search</v-icon>
                  <v-text-field
                    v-model="editedItem.rpmLandAmtDoc"
                    mask="##########/####"
                    return-masked-value
                    readonly
                    :label="'ค่าทดแทนที่ดิน (' + convertToMoneyFormat(editedItem.rpmLandAmt)+')'"
                  ></v-text-field>
                </v-layout>
              </v-flex>
              <v-flex xs12>
                <v-layout align-center justify-center row fill-height>
                  <v-icon @click="showChequeDetail(editedItem.rpmRemainAmtDoc)">search</v-icon>
                  <v-text-field
              ...
              <v-flex xs12>
                <v-layout align-center justify-center row fill-height>
                  <v-icon @click="showChequeDetail(editedItem.rpmBuildingAmtDoc)">search</v-icon>
                  <v-text-field
              ...
              <v-flex xs12>
                <v-layout align-center justify-center row fill-height>
                  <v-icon @click="showChequeDetail(editedItem.rpmBuildingRenovateAmtDoc)">search</v-icon>
                  <v-text-field
                    v-model="editedItem.rpmBuildingRenovateAmtDoc"
                    mask="##########/####"
                    return-masked-value
                    readonly
                    :label="'ค่าบูรณะ ('+convertToMoneyFormat(editedItem.rpmBuildingRenovateAmt)+' บาท)'"
                  ></v-text-field>
                </v-layout>
              </v-flex>
              <v-flex xs12>
                <v-layout align-center justify-center row fill-height>
                  <v-icon @click="showChequeDetail(editedItem.rpmPerennialAmtDoc)">search</v-icon>
                  <v-text-field
              ...
              <v-flex xs12>
                <v-layout align-center justify-center row fill-height>
                  <v-icon @click="showChequeDetail(editedItem.rpmRenterAmtDoc)">search</v-icon>
                  <v-text-field
                    v-model="editedItem.rpmRenterAmtDoc"
                    mask="##########/####"
                    return-masked-value
                    readonly
                    :label="'ค่าทดแทนผู้เช่า ('+convertToMoneyFormat(editedItem.rpmRenterAmtDoc)+')'"
                  ></v-text-field>
                </v-layout>
              </v-flex>
              <v-flex xs12>
                <v-layout align-center justify-center row fill-height>
                  <v-icon @click="showChequeDetail(editedItem.rpmSection21AmtDoc)">search</v-icon>
                  <v-text-field
                    v-model="editedItem.rpmSection21AmtDoc"
                    mask="##########/####"
                    return-masked-value
                    readonly
                    :label="'ค่าทดแทนม.21 วรรคท้าย ('+convertToMoneyFormat(editedItem.rpmSection21Amt)+')'"
                  ></v-text-field>
                </v-layout>
              </v-flex>
              <v-flex xs12>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" @click="close">ปิด</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>

    <v-dialog v-if="cheque[0]" v-model="dialog_cheque" max-width="800px">
      <v-card>
        <v-card-title>
          <span class="subheading">รายละเอียดการจ่าย</span>
        </v-card-title>
        <v-card-text>
          <v-container grid-list-md>
            <v-layout wrap>
              <v-flex xs3>
                <v-text-field v-model="cheque[0].chect" readonly :label="'เช็คเลขที่'"></v-text-field>
              </v-flex>
              <v-flex xs4>
                <v-text-field
                  readonly
                  :label="'จำนวนเงิน'"
                  :value="convertToMoneyFormat(cheque[0].rwbtr * -1)"
                ></v-text-field>
              </v-flex>
              <v-flex xs5>
                <v-text-field
                  v-model="cheque[0].bankAccounts[0].text1"
                  readonly
                  :label="'ธนาคาร/บัญชี'"
                ></v-text-field>
              </v-flex>
              <v-flex xs4>
                <v-text-field v-model="cheque[0].zaldt" readonly :label="'วันที่สั่งจ่าย'"></v-text-field>
              </v-flex>
              <v-flex xs4>
                <v-text-field v-model="cheque[0].pridt" readonly :label="'วันที่พิมพ์'"></v-text-field>
              </v-flex>
              <v-flex xs4>
                <v-text-field v-model="cheque[0].prius" readonly :label="'ผู้พิมพ์'"></v-text-field>
              </v-flex>
              <v-flex xs4>
                <v-text-field v-model="cheque[0].approve" readonly :label="'วันที่อนุมัติ'"></v-text-field>
              </v-flex>
              <v-flex xs4>
                <v-text-field v-model="cheque[0].issue" readonly :label="'วันที่รับเช็ค'"></v-text-field>
              </v-flex>
              <v-flex xs4>
                <v-text-field v-model="cheque[0].recipient" readonly :label="'ผู้รับเช็ค'"></v-text-field>
              </v-flex>
              <v-flex xs4>
                <v-text-field v-model="cheque[0].belnr" readonly :label="'Document จ่ายเช็ค'"></v-text-field>
              </v-flex>
              <v-flex xs4>
                <v-text-field v-model="cheque[0].bancd" readonly :label="'วันที่ขึ้นเงิน'"></v-text-field>
              </v-flex>
            </v-layout>
          </v-container>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" @click="dialog_cheque = false">ปิด</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-layout>
...
import SAPApi from "@/services/sap_api";
...
export default {
  ...
  methods: {
    ...
    showChequeDetail(data) {
      var docno = data.split("/")[0];
      var fiscalyear = data.split("/")[1];
      if (docno.length == 10 && fiscalyear.length == 4) {
        this.snackbar.open = true;
        this.snackbar.text = "กำลังเรียกข้อมูล, กรุณารอสักครู่ ...";
        SAPApi.getClearingDocument(docno, fiscalyear)
          .then(response => {
            this.snackbar.open = false;
            this.cheque = [];
            this.cheque = Object.assign({}, response.data.data.cheque);
        ...
        this.snackbar.open = true;
        this.snackbar.text = "กรุณาตรวจสอบข้อมูลเลขที่ใบสำคัญจ่ายอีกครั้ง";
      }
    },
    editItem(item) {
